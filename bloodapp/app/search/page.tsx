...
export default function DonorSearch() {
  ...
  const handleSearch = async () => {
    if (!bloodGroup || !district) {
      setPopup({ show: true, type: "warning", message: "कृपया ब्लड ग्रुप र जिल्ला दुवै चयन गर्नुहोस्।" });
      return;
    }
    ...
    setSearched(false);
    setDonors([]);
    setPopup({ show: true, type: "loading", message: "Donor search हुँदैछ...\nकृपया पर्खनुहोस्।" });

    const searchUrl = `${API_URL}?sheet=Donors&_=${new Date().getTime()}`;
    ...
    try {
      ...
      setPopup({ show: true, type: "error", message: "डाटा लोड गर्दा समस्या आयो।\nकृपया फेरि प्रयास गर्नुहोस्।" });
    } finally {
      setLoading(false);
    }
  };
      ...
      <div className="flex-grow p-6">
        <div className="max-w-xl mx-auto bg-white p-8 rounded-2xl shadow-lg border border-slate-100">
          ...
          <div className="space-y-4 mb-8">
            <select value={bloodGroup} onChange={(e) => setBloodGroup(e.target.value)} className="w-full p-3 border rounded-xl">
              <option value="">Select Blood Group</option>
              {["A+", "A-", "B+", "B-", "AB+", "AB-", "O+", "O-"].map((bg) => <option key={bg} value={bg}>{bg}</option>)}
            </select>
            <select value={district} onChange={(e) => setDistrict(e.target.value)} className="w-full p-3 border rounded-xl">
              <option value="">Select District</option>
              {[...districtList].sort().map((d) => <option key={d} value={d}>{d}</option>)}
            </select>
            <button onClick={handleSearch} disabled={loading} className="w-full bg-red-600 text-white p-3 rounded-xl font-bold hover:bg-red-700 transition disabled:bg-gray-400">
              {loading ? "Please wait..." : "Search Donors"}
            </button>
          </div>
